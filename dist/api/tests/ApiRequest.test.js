...
describe("ApiRequest", () => {
    describe("fetchJSON", () => {
        ...
        test("converts failed request to SDK error", (done) => {
            fetch.default.mockReturnValue(Promise.reject(new Error("forced error")));
            ApiRequest.fetchJSON("api/method", -1, { method: "POST" }).engage((error) => {
        ...
        test("converts failed request with JSON error into SDK Error", (done) => {
            fetch.default.mockReturnValue(Promise.resolve({
                ok: false,
                statusText: "not good",
                json: () => Promise.resolve([{ message: "API response error message" }]),
            }));
            ApiRequest.fetchJSON("api/method", -1, { method: "POST" }).engage((error) => {
        ...
        test("returns empty object if request status is a 204", (done) => {
            fetch.default.mockReturnValue(Promise.resolve({ ok: true, status: 204 }));
            ApiRequest.fetchJSON("api/method", -1, { method: "POST" }).engage((e) => done.fail(e), (response) => {
        ...
        test("returns special rate limiting error message when 429 response is returned", () => {
            ...
            ApiRequest.fetchJSON("group/get", -1, { method: "GET" }).engage((e) => {
                expect(e.code).toEqual(Constants_1.ErrorCodes.REQUEST_RATE_LIMITED);
    ...
    describe("createSignature", () => {
        test("return signature object with required fields", () => {
            const signature = ApiRequest.createSignature(12, "user-id-10", TestUtils_1.getSigningKeyPair());
