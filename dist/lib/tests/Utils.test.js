...
describe("Utils", () => {
    describe("Codec", () => {
        ...
        describe("PublicKey", () => {
            test("fromBase64", () => {
                expect(Utils.Codec.PublicKey.fromBase64({
                    x: "YXNhc2Vn",
                    y: "ZXNhZXQjJQ==",
                })).toEqual({
            ...
            test("toBase64", () => {
                expect(Utils.Codec.PublicKey.toBase64({
                    x: Buffer.from([97, 115, 97, 115, 101, 103]),
                    y: Buffer.from([101, 115, 97, 101, 116, 35, 37]),
                })).toEqual({
                    x: "YXNhc2Vn",
    ...
    describe("validateID", () => {
        test("throws when ID is not a string with length", () => {
            expect(() => Utils.validateID(3)).toThrow();
            expect(() => Utils.validateID(null)).toThrow();
            expect(() => Utils.validateID([])).toThrow();
            expect(() => Utils.validateID(["id-12"])).toThrow();
            expect(() => Utils.validateID("")).toThrow();
        });
    ...
    describe("validateDocumentData", () => {
        test("fails when data is not a string or byte array", () => {
            expect(() => Utils.validateDocumentData([])).toThrow();
            expect(() => Utils.validateDocumentData({})).toThrow();
            expect(() => Utils.validateDocumentData(10)).toThrow();
            expect(() => Utils.validateDocumentData("content")).toThrow();
    ...
    describe("validateEncryptedDocument", () => {
        test("fails when argument looks invalid", () => {
            expect(() => Utils.validateEncryptedDocument({})).toThrow();
            expect(() => Utils.validateEncryptedDocument({ foo: "bar" })).toThrow();
    ...
    describe("validateAccessList", () => {
        test("fails when no user or group list", () => {
            expect(() => Utils.validateAccessList({})).toThrow();
            expect(() => Utils.validateAccessList({ users: 3 })).toThrow();
            expect(() => Utils.validateAccessList({ users: null })).toThrow();
            expect(() => Utils.validateAccessList({ users: [] })).toThrow();
            expect(() => Utils.validateAccessList({ groups: 3 })).toThrow();
            expect(() => Utils.validateAccessList({ groups: null })).toThrow();
            expect(() => Utils.validateAccessList({ groups: [] })).toThrow();
        });
    ...
    describe("validateIDList", () => {
        test("fails when data is not an array or has no length", () => {
            expect(() => Utils.validateIDList({})).toThrow();
            expect(() => Utils.validateIDList("35")).toThrow();
            expect(() => Utils.validateIDList(3)).toThrow();
            expect(() => Utils.validateIDList([])).toThrow();
        });
        test("does not throw when data looks valid", () => {
    ...
    describe("generateDocumentHeaderBytes", () => {
        test("returns header with version byte and encoded metdata", () => {
            ...
            expect(versionHeader[0]).toEqual(2);
            expect(versionHeader[1]).toEqual(0);
