...
describe("StreamingAES", () => {
    describe("StreamingEncryption", () => {
        describe("getTransform", () => {
            test("should push IV onto stream as first option", () => {
                ...
                //We shouldn't push any more data if we already pushed on the IV and haven't gotten any new data
    ...
    describe("StreamingDecryption", () => {
        ...
        describe("getDecryptionStream", () => {
            test("returns expected Transform", () => {
                const sd = new StreamingDecryption(Buffer.alloc(32));
                expect(sd.getDecryptionStream()).toBeInstanceOf(Transform);
            });
        });
    });
});
